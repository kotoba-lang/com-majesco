(ns majesco.main
  "Kotodama WASM entrypoint for the Majesco clean-room actor (L4) — Clojure port.
  ...
  facts (`majesco.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "majesco")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "majesco_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}          :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "majesco_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"majesco.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "majesco-compat" :tier tier :entities entities} 200])
...
(defn start! [] :majesco-compat/ready)
